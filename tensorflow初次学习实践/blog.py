...
'''
这里输入的是一个1000000个元素的向量x以及1000000个元素的标签y
先将x和y进行批次（共batch_size批）的分割，每一批是独立的训练的样本，每批共1000000/batch_size=5000

之后我们由于使用截断传播，所以每次反向传播的长度是有限的，虽然一次输入了这一批5000个数据，
在5000里面，每num_steps个组成一个独立的rnn网络，共有5000/num_steps（epoch_size个，200个）他们的传播界限在这个网络里面，然后每个独立的state输出作为下一个num_steps大小的网络的输出

然后对这里面的每批x里的元素进行onehot转换，这里onehot的维度是2,
onehot之后，我们对该批次的数据进行了unstack也就是切割操作，数据数据变成了，该批次里面，各个样本的第一个输入放到了一起，第二个输入也放到了一起，以此类推，而不是之前各自独立，不过仍然是以onehot存在
之后我们取出该批次的所有的第一个输入的集合进行训练，更新state
'''
...
def gen_batch(raw_data, batch_size, num_step):
    ...
    epoch_size = batch_patition_length // num_steps  # ->5000/5=1000 就是每一轮的大小，（我认为这是截断传播的轮数，而不是大小）
